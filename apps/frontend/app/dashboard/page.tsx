    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span>Javob kutilayotganlar ({stats?.unansweredCount ?? 0})</span>
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-5 rounded-xl bg-surface border border-border-default shadow-xs flex items-center gap-4">
          ...
          <div>
            <span className="text-xs text-text-muted block">O‘rtacha javob vaqti</span>
            ...
            <p className="text-[11px] text-text-muted">
              Birinchi javobgacha o‘tgan vaqt
            </p>
          </div>
        </div>
        ...
        <div className="p-5 rounded-xl bg-surface border border-border-default shadow-xs flex items-center gap-4">
          ...
          <div>
            <span className="text-xs text-text-muted block">So‘nggi 24 soatda</span>
            <span className="text-lg font-bold text-accent-primary-dark">
              {statsLoading ? '...' : stats?.recentActivityCount ?? 0}
            </span>
            <p className="text-[11px] text-text-muted">
              Yangi xabar kelgan murojaatlar
            </p>
          </div>
        </div>
      </div>
              ...
              >
                <div className="space-y-1">
                  ...
                  <div className="text-[11px] text-text-muted flex items-center gap-3">
                    <span>
                      Kelgan {new Date(conv.createdAt).toLocaleTimeString('uz-UZ', { timeZone: 'Asia/Tashkent', hour: '2-digit', minute: '2-digit' })} ·{' '}
                      {new Date(conv.createdAt).toLocaleDateString('uz-UZ', { timeZone: 'Asia/Tashkent', month: 'short', day: 'numeric' })}
                    </span>
                    <span>•</span>
